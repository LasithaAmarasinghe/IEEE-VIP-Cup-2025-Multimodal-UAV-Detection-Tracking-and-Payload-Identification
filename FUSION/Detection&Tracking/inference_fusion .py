...
def process_single_video_file_with_fusion():
    import argparse
    parser = argparse.ArgumentParser(description="Process a video file for drone tracking")
    ...
    print(f"🤖 Models: {model_path_ir} (IR), {model_path_rgb} (RGB)")
    ...
    if not os.path.exists(model_path_ir) or not os.path.exists(model_path_rgb):
        print(f"❌ Model file(s) not found: {model_path_ir}, {model_path_rgb}")
        return

    # Load models
    try:
        model_cpu_rgb = YOLO(model_path_rgb)
        model_cpu_ir = YOLO(model_path_ir)
        model_gpu_rgb = YOLO(model_path_rgb).to('cuda') if torch.cuda.is_available() else model_cpu_rgb
        model_gpu_ir = YOLO(model_path_ir).to('cuda') if torch.cuda.is_available() else model_cpu_ir
    ...
    tracking_system = OptimizedDroneTrackingSystem(device='cuda', use_late_fusion=True)
    ...
    try:
        ...
        results = process_single_video_with_fusion(
            video_path, model_cpu_rgb, model_cpu_ir, model_gpu_rgb, model_gpu_ir, tracking_system
        )

        for frame, _ in enumerate(results):
            ret, frame_data = cap.read()
            if not ret:
                break
            tracking_system.draw_tracking_info(frame_data)
            out.write(frame_data)

        write_csv_data(results, csv_output_path)
        cap.release()
        out.release()
        print(f"\n✅ Video processing completed. Output: {csv_output_path}, {output_video_path}")
    except Exception as e:
